module.exports = function({ api, models }) {
	...
	const checkttDataPath = __dirname + '/../Naughty/commands/checktt/';
	setInterval(async() => {
...
(async function () {
		...
		try {
				logger(global.getText('listen', 'startLoadEnvironment'), '[ ÑAUGHTY-DATABASE ツ ]');
				...
				logger.loader(global.getText('listen', 'loadedEnvironmentUser')), logger(global.getText('listen','successLoadEnvironment'),'[ ÑAUGHTY-DATABASE ツ ]');
		} catch (error) {
				return logger.loader(global.getText('listen', 'failLoadEnvironment', error), 'error');
		}
}());
	logger(`${api.getCurrentUserID()} - [ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "THIS BOT MADE BY ÑAUGHTY ツ" : global.config.BOTNAME}`, "[ ÑAUGHTY-BOT INFO ツ ]");
	...
	//DEFINE DATLICH PATH
	const datlichPath = __dirname + '/../Naughty/commands/cache/datlich.json';
	...
	const checkAndExecuteEvent = async () => {
		...
		for (el of temp) {
			try {
				...
				var body = el.REASON || "EVERYBODY", mentions = [], index = 0;
			...
			if ("ATTACHMENT" in el) {
				...
				for (a of el.ATTACHMENT) {
					...
					fs.writeFileSync(__dirname + `/../Naughty/commands/cache/${a.fileName}`, Buffer.from(getAttachment, 'utf-8'));
					out.attachment.push(fs.createReadStream(__dirname + `/../Naughty/commands/cache/${a.fileName}`));
				}
			}
			console.log(out);
			if ("BOX" in el) await api.setTitle(el["BOX"], el["TID"]);
			api.sendMessage(out, el["TID"], () => ("ATTACHMENT" in el) ? el.ATTACHMENT.forEach(a => fs.unlinkSync(__dirname + `/../Naughty/commands/cache/${a.fileName}`)) : "");
		}
	...
	return (event) => {
		...
		let data = JSON.parse(fs.readFileSync(__dirname + "/../Naughty/commands/cache/approvedThreads.json"));
...
𝘿𝙚𝙖𝙧 𝘼𝙙𝙢𝙞𝙣 𝙏𝙝𝙞𝙨 𝘽𝙤𝙩 𝙄𝙙 𝙍𝙚𝙦𝙪𝙚𝙨𝙩𝙚𝙙 𝘼𝙥𝙥𝙧𝙤𝙫𝙖𝙡. 
...
𝘿𝙚𝙖𝙧 𝙐𝙨𝙚𝙧𝙨. 
											 𝙮𝙤𝙪𝙧 𝙍𝙚𝙦𝙪𝙚𝙨𝙩 𝙃𝙖𝙨 𝘽𝙚𝙚𝙣 𝙎𝙪𝙘𝙘𝙚𝙨𝙨𝙛𝙪𝙡𝙡𝙮 𝙎𝙚𝙣𝙩 𝙏𝙤 𝘽𝙤𝙩 𝘼𝙙𝙢𝙞𝙣(𝙨) . 

𝙉𝙤𝙬 𝙒𝙖𝙞𝙩 𝙐𝙣𝙩𝙞𝙡 𝘽𝙤𝙩 𝘼𝙙𝙢𝙞𝙣𝙨 𝘼𝙥𝙥𝙧𝙤𝙫𝙚 𝙔𝙤𝙪𝙧 𝙍𝙚𝙦𝙪𝙚𝙨𝙩.... 
...
𝙔𝙤𝙪𝙧 𝙂𝙧𝙤𝙪𝙥 𝙄𝙨𝙣'𝙩 𝘼𝙥𝙥𝙧𝙤𝙫𝙚𝙙

•.:°❀×═════════×❀°:.•

𝙏𝙮𝙥𝙚 "+𝙧𝙚𝙦𝙪𝙚𝙨𝙩" 𝙄𝙣 𝙔𝙤𝙪𝙧 𝙂𝙧𝙤𝙪𝙥 𝙏𝙤 𝙍𝙚𝙦𝙪𝙚𝙨𝙩 𝘼𝙥𝙥𝙧𝙤𝙫𝙖𝙡 𝙁𝙧𝙤𝙢 𝘽𝙤𝙩 𝘼𝙙𝙢𝙞𝙣(𝙨) 
