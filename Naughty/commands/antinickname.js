...
writeFileSync(pathxData, JSON.stringify(dataJson, null, 4), "utf-8");
 ...
 const { join } = require('path');
 const pathData = join(__dirname, "cache", "abboxname.json");
...
const pathData = join(__dirname, "cache", "abboxname.json");
  ...
  body:`[⚜️]=== 『 UPDATE SUCCESS 』 ===[⚜️]\n━━━━━━━━━━━━━━━━\n[⚜️]➜ Đã ${args[0] == "boxname" ? Thread_1.boxname : args[0] == "username" ? Thread_1.username : Thread_1.avtbox} ${args[0] == "boxname" ? "boxname" : "cấm đổi biệt danh" } thành công!`, attachment: (await axios.get((await axios.get(`https://docs-api.jrtxtracy.repl.co/images/wallpaper?apikey=JRTvip_2200708248`)).data.data, {
                  responseType: 'stream'
              })).data
