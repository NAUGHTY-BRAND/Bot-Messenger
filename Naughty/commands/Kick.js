...
module.exports.run = async function({ api, args, Users, event, Threads, utils, client }) {
	...
	if (!info.adminIDs.some(item => item.id == api.getCurrentUserID())) return api.sendMessage('⏤͟͟͞͞✰𝙄 𝘼𝙢 𝙉𝙤𝙩 𝙂𝙧𝙤𝙪𝙥 𝘼𝙙𝙢𝙞𝙣 𝙎𝙤 𝙁𝙞𝙧𝙨𝙩 𝘼𝙙𝙙 𝙈𝙚 𝘽𝙤𝙭 𝘼𝙙𝙢𝙞𝙣 𝘼𝙣𝙙 𝙏𝙝𝙚𝙣 𝙏𝙧𝙮 𝘼𝙜𝙖𝙞𝙣⁂', threadID, messageID);
  ...
  */
  ...
  if(args[0] == "view") {
  	if(!args[1]) {
  		...
  		if(!mywarn) return api.sendMessage('❒❑𝙔𝙤𝙪 𝙃𝙖𝙫𝙚 𝘽𝙚𝙚𝙣 𝙒𝙖𝙧𝙣𝙚𝙙', threadID, messageID);
  		var num = 1;
  		for(let reasonwarn of mywarn) {
  			msg += `reasonwarn\n`;
  		}
  		api.sendMessage(`❥𝙔𝙤𝙪 𝙃𝙖𝙫𝙚 𝘽𝙚𝙚𝙣 𝙒𝙖𝙧𝙣𝙚𝙙 𝙁𝙤𝙧 𝙏𝙝𝙚 𝙍𝙚𝙖𝙨𝙤n┆ ${msg}`, threadID, messageID);
  	}
  		...
  		allwarn == "" ? api.sendMessage("✰𝙉𝙤 𝙊𝙣𝙚 𝙄𝙣 𝙔𝙤𝙪𝙧 𝙂𝙧𝙤𝙪𝙥 𝙃𝙖𝙨 𝘽𝙚𝙚𝙣 𝙒𝙖𝙧𝙣𝙚𝙙 𝙄𝙣 𝙔𝙤𝙪𝙧 𝙂𝙧𝙤𝙪𝙥 𝙔𝙚𝙩", threadID, messageID) : api.sendMessage("⁂𝙇𝙞𝙨𝙩 𝙊𝙛 𝙈𝙚𝙢𝙗𝙚𝙧𝙨 𝙒𝙝𝙤 𝙃𝙖𝙫𝙚 𝘽𝙚𝙚𝙣 𝙒𝙖𝙧𝙣𝙚𝙙"+allwarn, threadID, messageID);
  	}
  }
	...
	if (!info.adminIDs.some(item => item.id == senderID) && !(global.config.ADMINBOT).includes(senderID)) return api.sendMessage('❎Right cunt border!', threadID, messageID);
  	...
  	if(!mybox.includes(id)) return api.sendMessage("✅This person hasn't been banned from your group yet", threadID, messageID);
			api.sendMessage(`❍𝙎𝙪𝙘𝙚𝙨𝙨𝙛𝙪𝙡𝙡𝙮 𝙍𝙚𝙢𝙤𝙫𝙚𝙙 ❦`, threadID, messageID);
  ...
  var reason = "";
		...
		api.sendMessage({body: `\n╞═════𖠁𐂃𖠁═════╡\n\n➬𝙍𝙚𝙢𝙤𝙫𝙚𝙙 𝙎𝙪𝙘𝙚𝙨𝙨 ${arrayname.join(", ")} \n\n╞═════𖠁𐂃𖠁═════╡`, mentions: arraytag}, threadID, messageID);
		fs.writeFileSync(__dirname + `/cache/bans.json`, JSON.stringify(bans, null, 2));
}
