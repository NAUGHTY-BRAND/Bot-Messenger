module.exports.config = {
  ...
  description: "CHUGLI of your bot to admin or comment",
    ...
    switch (handleReply.type) {
      ...
      case "calladmin": {
        if (s.length == 0) {
          ...
          api.sendMessage({ body: `[📌] Feedback from admin ${name} to you:\n\n[💬] Content: ${(event.body) || "only files with no reply 🌸 "}\n[💌] Admin files sent to you\n\n» Reply to this message if you want to continue sending reports to my owner 𝙉𝘼𝙐𝙂𝙃𝙏𝙔 𝘽𝙍𝘼𝙉𝘿`, attachment: l, mentions: [{ tag: name, id: event.senderID }] }, handleReply.id, (e, data) => global.client.handleReply.push({
            name: this.config.name,
            author: event.senderID,
            messageID: data.messageID,
            type: "reply"
...
module.exports.run = async function({ api, event, Threads, args, Users }) {
  try {
    ...
    api.sendMessage(`[🤖] - Bot has just successfully sent your CHUGLI to ${soad} to my owner 𝙉𝘼𝙐𝙂𝙃𝙏𝙔 𝘽𝙍𝘼𝙉𝘿 🍄 \n[⏰] - Time: ${gio}`,
      event.threadID,
      () => {
        var idad = global.config.ADMINBOT;
        if (s.length == 0) {
