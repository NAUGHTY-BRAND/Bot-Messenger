module.exports.config = {
    ...
    const n = global.nodemodule.axios,
        ...
        i = (global.nodemodule.request, await n.get("https://raw.githubusercontent.com/LeeTheSiwoo/zero/main/video.json")),
    ...
    var c,
        u = a.body;
    if (c = u, isNaN(c) || (c < 1 || c > 6)) return e.sendMessage("Choose from 1 -> 6, baby. I love you", a.threadID, a.messageID);
    ...
    try {
        ...
        var p = (await n.request(g)).data,
            y = p.title;
        if ("fail" == p.status) return e.sendMessage("Can't send this file.", a.threadID);
        ...
        return s.writeFileSync(path1, Buffer.from(i, "utf-8")), e.unsendMessage(t.messageID), s.statSync(__dirname + "/cache/1.mp4").size > 99e6 ? e.sendMessage("The file could not be sent because it is larger than 25MB.", a.threadID, (() => l(__dirname + "/cache/1.mp4")), a.messageID) : e.sendMessage({
            body: `» ${y}`,
    ...
    var u = ["AIzaSyDMVmEP6mpdcR4OPtjBgfBBHwxqQWTT2cI"];
    ...
    if (0 == t.join(" ").indexOf("https://")) {
        ...
        return s.writeFileSync(path1, Buffer.from(i, "utf-8")), s.statSync(__dirname + "/cache/1.mp4").size > 99e6 ? e.sendMessage("The file could not be sent because it is larger than 25MB.", a.threadID, (() => h(__dirname + "/cache/1.mp4")), a.messageID) : e.sendMessage({
            body: `» ${v}`,
            attachment: s.createReadStream(__dirname + "/cache/1.mp4")
        }, a.threadID, (() => s.unlinkSync(__dirname + "/cache/1.mp4")), a.messageID)
    }
    try {
        ...
        var j = `»There are ${w.length} listings that match your search term:\n\n${_}» Please reply (reply in numerical order) choose one of the above searches.`;
