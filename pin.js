  ...
  if (args[0] == "down") {
    ...
    try {
      ...
      if (!isNaN(args[args.length - 1])) {
        ...
        if (limit > 50)
          return api.sendMessage("[⚜️] ➜ 𝗕𝗮̣𝗻 𝗰𝗵𝗶̉ 𝗰𝗼́ 𝘁𝗵𝗲̂̉ 𝘁𝗮̉𝗶 𝗻𝗵𝗶𝗲̂̀𝘂 𝗻𝗵𝗮̂́𝘁 𝟱𝟬 𝗮̉𝗻𝗵", event.threadID, event.messageID);
        keyword = event.messageReply ? keyword : keyword.replace(/(\d+)$/, "").trim();
      }

      if (!keyword)
        return api.sendMessage('[⚜️] ➜ 𝗩𝘂𝗶 𝗹𝗼̀𝗻𝗴 𝗻𝗵𝗮̣̂𝗽 𝘁𝘂̛̀ 𝗸𝗵𝗼𝗮́ 𝘁𝗶̀𝗺 𝗸𝗶𝗲̂́𝗺 🔎', event.threadID, event.messageID);

      const url = `${global.configApi.domain1}/pinterest?search=${encodeURIComponent(keyword)}`;
      const { data } = await axios.get(url);
      let results = data.data;
      if (data.length == 0)
        return api.sendMessage(`[⚜️] ➜ 𝗞𝗵𝗼̂𝗻𝗴 𝗰𝗼́ 𝗸𝗲̂́𝘁 𝗾𝘂𝗮̉ 𝘁𝗶́𝗺 𝗸𝗶𝗲̂́𝗺 𝗻𝗮̀𝗼 𝗰𝗵𝗼 𝘁𝘂̛̀ 𝗸𝗵𝗼́𝗮: ${keyword}`, event.threadID, event.messageID);
      ...
      return api.sendMessage({
        body: `🌸=== [ 𝗣𝗜𝗡𝗧𝗘𝗥𝗘𝗦𝗧 ] ===🌸\n━━━━━━━━━━━━━\n\n[⚜️] ➜ 𝗖𝗼́ ${getAll.length} 𝗸𝗲̂́𝘁 𝗾𝘂𝗮̉ 𝘁𝗶̀𝗺 𝗸𝗶𝗲̂́𝗺 𝗮̉𝗻𝗵 𝘁𝗿𝗲̂𝗻 𝗽𝗶𝗻𝘁𝗲𝗿𝗲𝘀𝘁 𝗰𝘂̉𝗮 𝘁𝘂̛̀ 𝗸𝗵𝗼𝗮́ ${keyword} 🌸\n` + (limit && limit > getAll.length ? `[⚜️] ➜ Đã xảy ra lỗi khi tải ${limit - getAll.length} ảnh` : ""),
        attachment: getAll
      }, event.threadID, event.messageID);
    }
    catch (e) {
      console.log(e);
      return api.sendMessage("[⚜️] ➜ Đã có lỗi xảy ra", event.threadID, event.messageID);
    }
  }
  else {
    return api.sendMessage({body:`🌸==『 𝗣𝗜𝗡𝗧𝗘𝗥𝗘𝗦𝗧 』==🌸\n\n[⚜️] ➜ 𝗕𝗮̣𝗻 𝗰𝗼́ 𝘁𝗵𝗲̂̉ 𝗱𝘂̀𝗻𝗴 𝗻𝗵𝘂̛̃𝗻𝗴 𝗰𝗵𝘂̛́𝗰 𝗻𝗮̆𝗻𝗴 𝘀𝗮𝘂:\n[🔎] ➜ ${global.config.PREFIX}${this.config.name} 𝘀𝗲𝗮𝗿𝗰𝗵: 𝗧𝘂̛̀ 𝗸𝗵𝗼𝗮́ 𝘁𝗶̀𝗺 𝗸𝗶𝗲̂́𝗺 - 𝘀𝗼̂́ 𝗮̉𝗻𝗵\n[🔰] ➜ ${global.config.PREFIX}${this.config.name} 𝗱𝗼𝘄𝗻 + 𝗹𝗶𝗻𝗸: 𝘁𝗮̉𝗶 𝗮̉𝗻𝗵/𝘃𝗱 𝗰𝗼́ 𝗰𝗵𝘂̛́𝗮 𝗹𝗶𝗻𝗸`, attachment: fs.createReadStream(__dirname + `/noprefix/pinterest.jpg`) }, event.threadID, event.messageID);
  }
};
